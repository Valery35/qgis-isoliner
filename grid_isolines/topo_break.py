...
def edge_strength(z, cell, nodata_mask=None):
    ...
    # двух ячеек не определён честно и гасится.
    e[:2, :] = 0.0
    e[-2:, :] = 0.0
    e[:, :2] = 0.0
    e[:, -2:] = 0.0
...
def auto_thresholds(e, ridge):
    """Пороги гистерезиса из распределения признака на гребнях.

    Медиана признака по гребням служит шумовым полом: на плотной съёмке
    шумовых гребней на порядок больше настоящих, и любой процентиль без
    привязки к полу садится в шум. Верхний порог - шесть полов, но не ниже
    75-го процентиля (на чистой синтетике гребни только настоящие, и там
    работает процентиль). Нижний - половина верхнего. Пороги по-прежнему
    мягкие: их работа отсечь фон, значимость решает перепад и человек.
    """
    vals = e[ridge & (e > 0.0)]
    if vals.size == 0:
        return 0.0, 0.0
    floor = float(np.median(vals))
    t_high = max(6.0 * floor, float(np.percentile(vals, 75.0)))
    # вырожденный случай чистой синтетики: все гребни с одинаковым
    # признаком, медиана равна максимуму, и шесть полов улетают выше него.
    # Порог не имеет права превышать сам признак.
    t_high = min(t_high, 0.8 * float(vals.max()))
...
def trace_lines(mask):
    ...
    больше прямого угла - честный конец линии. Возвращает список списков
    (row, col).
    """
...
def breakline_candidates(z, cell, min_drop=0.2, min_len_cells=5,
    ...
    t_low, t_high = auto_thresholds(e, ridge)
...
def pair_breaklines(brows, toes, z, cell, max_dist=200.0, min_share=0.4,
    ...
    признака), и все куски честно спускаются к одной и той же подошве.
