...
def level_key(x, y, quant=1e-6):
    """Ключ вершины для сверки с линиями уровня.

    Координаты вершин пояса приходят из полигонизации тех же линий и
    совпадают с их вершинами точно, но округление до микрометра снимает
    расхождение в последнем бите после объединения геометрий.
    """
    return (round(float(x) / quant), round(float(y) / quant))


def vertex_levels(xs, ys, arr, gt, lo, hi):
    """Отметка вершины в границах пояса: выборка поверхности с прижатием.

    Нужна тому, кто кладёт линию или точку на поверхность внутри
    известного диапазона. Контурные полигоны 1.04 ею не пользуются: их
    кольца плоские, иначе движок триангулирует неплоское кольцо в плане
    и натягивает через весь пояс длинные тонкие треугольники.

    Выборка идёт билинейно, а ближайшей ячейкой только там, где
    билинейная пуста: она пуста на самом краю растра, а контур области
    идёт ровно по краю.
    """
    ...
    if not cand:
        return None
    if len(cand) == 1:
        return np.full(xs.shape, cand[0])
    ...
    still = ~np.isfinite(z)
    if still.any():
        z[still] = 0.5 * (a_lo + a_hi)
    return np.clip(z, a_lo, a_hi)
