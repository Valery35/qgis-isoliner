...
def test_page_names_no_tool_count():
    """Числа инструментов на странице нет вовсе.

    Прежде здесь стояла сверка «инструментов в модуле N» с модулем. Она
    требовала держать число в тексте руками, и число всё равно отставало:
    страница обещала семьдесят при семидесяти одном. Правило изменено -
    число называет только дерево, собранное генератором, а в тексте его
    нет. Сверка дерева и запрет на рукописное число живут в
    `tests/test_tool_count.py`, здесь остаётся контроль, что число не
    вернулось на страницу.
    """
    page = _page()
    if page is None:
        print("   (страницы быстрого старта нет, проверка пропущена)")
        return
    m = re.search(r"нструментов в модуле (\d+)", page)
    assert not m, (
        "на страницу вернулось число инструментов (%s). Оно устареет на "
        "следующем инструменте, а читатель считает его обещанием"
        % (m.group(1) if m else ""))
