...
TRANSLATIONS = {
    ...
    'Геологический разрез по линии из набора поверхностей. Поверхности задаются списком и упорядочиваются сверху вниз (кровля, подошва, следующая кровля и так далее). Пласты строятся как полосы между соседними поверхностями, поэтому N поверхностей дают N−1 пластов.\n\nПо умолчанию порядок поверхностей берётся из дерева слоёв проекта (сверху вниз), совпадая с панелью слоёв. Снимите галочку **Порядок поверхностей из дерева слоёв проекта**, чтобы задать порядок вручную отметками в списке.\n\nПо умолчанию разрез строится по каждой линии слоя. Все разрезы попадают в один комплект слоёв и различаются полями **sec** (имя) и **sec_id**, поэтому подписи и фильтры делаются выражением по полю, а стили не назначаются заново. Имя берётся из поля линии, если оно задано, иначе нумеруется по порядку.\n\nНа чертеже разрезы разводятся раскладкой в общей чертёжной системе координат. Стопкой - у всех общий ноль расстояний, в ряд - общая отметка высоты, сеткой - и то и другое по строкам и столбцам. Шаг решётки берётся от самого крупного габарита плюс зазор, поэтому чертежи не пересекаются по построению. Вертикальный масштаб на весь прогон один, иначе разрезы были бы несопоставимы.\n\nВертикальный масштаб задаётся тремя способами. Отношение масштабов Г:В (1:N) - привычная для чертежа запись, вертикальный масштаб крупнее горизонтального в N раз. Множитель - то же число напрямую. Отношение габаритов чертежа - множитель подбирается так, чтобы ширина относилась к высоте как задано.\n\nДва выхода. Чертёж разреза - полигоны в осях расстояние вдоль линии и высота, с вертикальным преувеличением для макета и печати. Забор 3D - те же полосы как вертикальные стенки PolygonZ в реальных координатах, для просмотра в 3D Map View рядом с поверхностями. Забор раскладкой не смещается, он стоит в реальных координатах.\n\nПоверхности обычно получают кригингом (кровля, подошва пласта). Линию рисуют как обычный линейный слой. Расстояние и высота берутся в единицах карты. Свой кригинг инструмент не выполняет.': 'Geological cross-section along a line from a set of surfaces. The surfaces are given as a list and ordered top to bottom (roof, floor, next roof and so on). Beds are built as bands between neighbouring surfaces, so N surfaces give N-1 beds.\n\nBy default the order of the surfaces is taken from the project layer tree (top to bottom), matching the Layers panel. Uncheck **Surface order from the project layer tree** to set the order manually by the ticks in the list.\n\nBy default a section is built for every line of the layer. All sections go into one set of layers and are told apart by the **sec** (name) and **sec_id** fields, so labels and filters are made by an expression on the field and styles are not reassigned. The name comes from the line field if one is set, otherwise the sections are numbered in order.\n\nOn the drawing the sections are separated by a layout in the common drawing coordinate system. Stacked - all share the zero of distance, in a row - all share the elevation datum, in a grid - both, by rows and columns. The lattice pitch comes from the largest extent plus the gap, so the drawings cannot overlap by construction. The vertical scale is common to the whole run, otherwise the sections would not be comparable.\n\nThe vertical scale is set in three ways. The scale ratio H:V (1:N) is the usual drawing notation, the vertical scale being N times larger than the horizontal one. The factor is the same number directly. The drawing extent ratio picks the factor so that the width is to the height as requested.\n\nTwo outputs. The section drawing is polygons in axes of distance along the line and elevation, with vertical exaggeration for layout and printing. The 3D fence is the same bands as vertical PolygonZ walls in real coordinates, for viewing in the 3D Map View next to the surfaces. The fence is not moved by the layout, it stands in real coordinates.\n\nThe surfaces are usually obtained by kriging (bed roof and floor). The line is drawn as an ordinary line layer. Distance and elevation are taken in map units. The tool performs no kriging of its own.',
    'Гистограмма невязок': 'Histogram of residuals',
    'Горизонтали (линии)': 'Contours (lines)',
    'Горизонтали для определения сечения (необязательно)': 'Contours to detect the interval (optional)',
    ...
    'Канал ЦМР': 'DEM band',
    'Куда положить (охват, необязательно)': 'Where to place it (extent, optional)',
    ...
    'Поле имени разреза (необязательно)': 'Section name field (optional)',
