...
TRANSLATIONS = {
    ...
    'Справочник пластов (слой), необязательно': 'Bed reference (layer), optional',
    ...
    'Цвета полос: из палитры %d, без цвета %d, своих %d.': 'Band colours: %d from the palette, %d without a colour, %d own.',
    'Записывает палитру кодов Leapfrog (.lfc) из раскраски слоя. Слой должен быть раскрашен категориями (Свойства - Стиль - Категорированный): значение категории становится кодом, её цвет - цветом кода, порядок категорий сохраняется.\n\nТакую палитру принимают инструменты 4.01 и 4.02, поэтому цвета чертежа можно задать прямо в QGIS, без Leapfrog: раскрасили слой руками, выгрузили палитру, подали её в разрезы. Файл читается и самим Leapfrog.\n\nС плагином поставляются готовые палитры (%s) в папке palettes внутри каталога плагина: example.lfc показывает формат, demo.lfc покрывает коды демо-разреза, остальные - рабочие легенды Верхнекамского месторождения (пласты, слои, породы), опубликованные с разрешения правообладателя.': 'Writes a Leapfrog colour palette (.lfc) from the styling of a layer. The layer must be styled by categories (Properties - Symbology - Categorized): the category value becomes the code, its colour becomes the colour of the code, and the order of the categories is kept.\n\nSuch a palette is accepted by tools 4.01 and 4.02, so the colours of a section drawing can be set right in QGIS without Leapfrog: style a layer by hand, export the palette, feed it into the sections. The file is also read by Leapfrog itself.\n\nReady palettes ship with the plugin (%s) in the palettes folder inside the plugin directory: example.lfc shows the format, demo.lfc covers the codes of the section demo, and the rest are working legends of the Verkhnekamskoye deposit (beds, layers, rocks), published with the permission of the rights holder.',
    'Слой с категорированной раскраской': 'Layer styled by categories',
    'Палитра кодов Leapfrog (.lfc)': 'Leapfrog colour palette (.lfc)',
    'Палитра Leapfrog (*.lfc)': 'Leapfrog palette (*.lfc)',
    ...
    '2.16 Водосбор линии и контура (канавы, карьеры)': '2.16 Catchment of a line or an outline (ditches, open pits)',
    ...
    'Палитра кодов Leapfrog (.lfc), необязательно': 'Leapfrog colour palette (.lfc), optional',
    'Палитра Leapfrog (*.lfc *.LFC);;XML (*.xml);;Все файлы (*.*)': 'Leapfrog palette (*.lfc *.LFC);;XML (*.xml);;All files (*.*)',
    'Палитра не прочитана (%s), цвета считаются от кодов.': 'The palette was not read (%s), colours are computed from the codes.',
    'Цвета: из палитры %d кодов, своих %d.': 'Colours: %d codes from the palette, %d own.',
    'Цвета полос: из палитры %d, своих %d.': 'Band colours: %d from the palette, %d own.',
    'Нет в палитре: %s': 'Not in the palette: %s',
    'Палитра: принято %d из %d записей.': 'Palette: %d of %d entries accepted.',
    ...
    'Кладёт скважины на чертежи разрезов из пары слоёв модели бурения: устья collar (hole_id, z, eoh, точки) и таблица интервалов interval (hole_id, from, to, code, глубины по стволу от устья). Такую пару выдают «Создать пример для разреза» и выгрузка из Геоконструктора. Поля обеих таблиц находятся сами по контрактным именам, выбор полей спрятан в дополнительные параметры.\n\nЛинии, вертикальный масштаб и раскладка берутся из определения разреза, которое выдаёт «Разрез по линии»: один прогон кладёт колонки сразу на все чертежи, каждая скважина попадает на все разрезы, к которым она ближе коридора. Глубины переводятся в отметки вычитанием из z.\n\nЧитатель терпимый: пустые глубины пропускаются, перепутанные from и to меняются местами, перехлёсты и интервалы за забоем рисуются как есть, всё пропущенное считается и выводится сводкой в журнал. Прочие колонки таблицы интервалов едут в атрибуты как есть.\n\nПодпись устья берётся из поля number (или name, label), а без него из hole_id. Поле можно указать в дополнительных параметрах.\n\nПо умолчанию колонки обрезаются рамкой чертежа zmin и zmax из определения: интервал на кромке подрезается, интервал целиком за рамкой пропускается, ствол и подпись зажимаются рамкой. Допуск в дополнительных параметрах расширяет рамку, галочка выключает обрезку совсем. В атрибутах ztop и zbot остаются настоящие отметки интервала, обрезается только геометрия.\n\nЧертёж разреза - необязательный вход с полигонами полос из 4.01: колонки режутся по верхней и нижней огибающей полос в своей позиции, скважины не вылезают за чертёж. Колонка за краем полос этим входом не режется, там работает рамка.\n\nЦвет интервала можно взять из палитры кодов Leapfrog - файла .lfc с парами код и цвет. Коды, найденные в палитре, красятся её цветами, порядок категорий в легенде идёт по палитре, а коды вне палитры сохраняют свой детерминированный цвет. Без палитры всё работает как раньше. Готовые палитры лежат в папке palettes внутри каталога плагина. Палитра нужна один раз: получив цвета, сохраните стиль слоя в .qml и дальше работайте штатными средствами QGIS.': 'Places boreholes onto section drawings from a drilling-model pair: collars (hole_id, z, eoh, points) and an interval table (hole_id, from, to, code, downhole depths from the collar). Such a pair comes from Create a section example and from the Geoconstructor export. The fields of both tables are found automatically by the contract names, and the field pickers live under the advanced parameters.\n\nThe lines, vertical exaggeration and layout come from the section definition produced by Cross-section along a line: one run places columns on all drawings at once, and each borehole lands on every section closer than the corridor. Depths become elevations by subtraction from z.\n\nThe reader is tolerant: empty depths are skipped, swapped from and to are exchanged, overlaps and intervals beyond eoh are drawn as is, and everything skipped is counted and reported in the log. Other interval table columns travel into attributes as is.\n\nThe collar label comes from the number field (or name, label), and without it from hole_id. The field can be set under the advanced parameters.\n\nBy default the columns are clipped by the drawing frame zmin and zmax from the definition: an interval on the edge is trimmed to it, an interval entirely beyond the frame is skipped, the trace and the label are clamped by the frame. The tolerance under the advanced parameters widens the frame, and the checkbox turns clipping off entirely. The ztop and zbot attributes keep the true interval elevations, only the geometry is clipped.\n\nThe section drawing is an optional input with the band polygons from 4.01: the columns are cut by the upper and lower envelope of the bands at their own position, so boreholes do not stick out of the drawing. A column beyond the bands is not cut by this input, the frame works there.\n\nThe interval colour can be taken from a Leapfrog colour palette - an .lfc file with code and colour pairs. Codes found in the palette are painted with its colours, the order of the legend categories follows the palette, and codes outside the palette keep their own deterministic colour. Without a palette everything works as before.\n\nReady palettes live in the palettes folder inside the plugin directory. The palette is needed once: having got the right colours and labels, save the layer style as .qml and work with the standard QGIS means from there on.',
