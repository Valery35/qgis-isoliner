...
def _warn_flat_levels(raster, band, interval, base, levels, feedback,
    ...
    try:
        ...
        feedback.pushInfo(_tr(
            "Обычно это водная поверхность или залитая площадка. Замаскируйте её до "
...
def _belts_to_layer(processing, polys_src, arr, valid, gt, levels, crs,
    ...
    if n_fixed:
        feedback.pushInfo(_tr(
            "Поясов, где выборка растра дала бы соседний диапазон: %d. Диапазон взят "
            "из уровней ограничивающих изолиний. Так бывает на поясе тоньше ячейки. "
            "Изолиния проведена интерполяцией между центрами ячеек, а выборка отдаёт "
            "значение самой ячейки.") % n_fixed)
    if n_orphan:
        feedback.pushInfo(_tr(
    ...
    if not solid_feats:
        feedback.pushWarning(_tr(
            "Тел не построено, у поясов нулевая мощность или пустые кольца."))
        return {"belts": belts, "solids": None}
    sdp.addFeatures(solid_feats)
    smem.updateExtents()
    feedback.pushInfo(_tr(
        "Тел построено: %d. Каждое замкнуто: крышка снизу на ELEV_MIN, сверху на "
        "ELEV_MAX, стенки по всем кольцам, внутренним тоже.")
        % len(solid_feats))
    return {"belts": belts,
