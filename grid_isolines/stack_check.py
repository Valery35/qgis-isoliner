...
существовании соседей. Пока пласты выдержаны, это остаётся незаметным. В зоне
...
CODE_CUT = 5       # тело срезано: его кровля совпала с зеркалом


def cut_mask(top, mirror, tol=0.5):
    """Где кровля тела совпала с уровнем растворения, то есть срезана.

    Без зеркала срез и выклинивание неразличимы: и там, и там мощность
    уходит в ноль. Разница в том, что при выклинивании кровля и подошва
    сходятся друг к другу, а при срезе кровля лежит на зеркале, и выше
    тела просто нет. Поэтому код среза выдаётся только при поданном
    уровне растворения, а без него зона остаётся выклиниванием.
    """
    t = np.asarray(top, dtype=float)
    m = np.asarray(mirror, dtype=float)
    d = np.abs(t - m)
    return np.isfinite(d) & (d <= float(tol))
