...
Без охвата демо создаётся в условном месте, одном и том же, чтобы примеры руководства воспроизводились. В местной системе координат, например рудничной, это место оказывается далеко от рабочих данных, и на карте вы увидите пустоту. Задайте параметр **Куда положить (охват)**, и рельеф ляжет туда, а размер грида посчитается от охвата и размера ячейки.
...
Допуск в дополнительных параметрах расширяет рамку и полосы наружу, в единицах отметки. Обрезается только геометрия, в атрибутах ztop и zbot остаются настоящие отметки интервала. Сводка обрезки печатается в журнал по каждому разрезу.
...
| Скважины вдоль линии | Точки с полями h1...h6 (широкий формат, задел под конвертер). | по запросу |
