...
- `6.01` Фрактальная размерность
- `6.02` Box-counting маски
- `6.03` Размерность линий и границ
- `6.04` Размерность Минковского (векторы)
- `6.05` Создать пример для фракталов (демо)

_Всего инструментов: 59_
<!-- /TREE -->
...
После установки инструменты появляются в панели «Обработка» (Processing), провайдер «Isoliner», группы «Грид и изолинии», «Топография», «Дополнительные инструменты анализа», «Разрез», «Геологическая модель» и «Фрактальный анализ». Требования: QGIS 3.16+. Внешних зависимостей нет - используются NumPy, GDAL и штатные алгоритмы Processing из состава QGIS.
...
Кровля пласта из кригинга подаётся в **6.01** с окном 8, полученный грид D - в **1.04 Изолинии из растра** (канал 1), и поверх структурного плана ложатся изолинии размерности с поясами. Глобальное D из журнала - одно число на поверхность, по нему можно сравнивать участки или пласты между собой. Растр должен быть в метрической системе координат. Демо-поверхности подходят как есть.
...
Метод дополняет циркуль из **6.03**: циркуль меряет извилистость одной линии, Минковский - заполнение плоскости набором объектов. Размерность возвращается и числовым выходом для моделей Processing.
...
Генератор учебных объектов для всей фрактальной группы: ветвящаяся речная сеть с полем order (порядок притока), полигон водосбора с изрезанной границей и отдельная береговая линия, построенная срединными смещениями. Реки можно подать в **6.04** и получить размерность сети. Берег и границу водосбора - в **6.03** и **6.04**, и сравнить циркуль с Минковским. Водосбор растеризуется штатным инструментом и служит примером для **6.02**.
...
## Данные для фракталов - инструмент 6.05
