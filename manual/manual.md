...
![Инструменты модуля в панели «Обработка»: провайдер Isoliner с тремя группами - «1. Грид и изолинии» (1.1-1.7), «2. Дополнительные инструменты анализа» (2.1-2.5) и «3. Разрез» (3.1-3.2).](images/ui_toolbox.png){width=55%}
...
# Инструмент «Кригинг с внешним дрейфом (External Drift)»
...
# Инструмент «Карта вероятности превышения»
...
# Инструмент «Удельный расход (закон Дарси)»
...
# Инструмент «Гауссова симуляция (SGS)»
...
Параметры. **Поле значения** - числовой атрибут. **Количество реализаций** - сколько прогонов усреднять, разумное начало 50-100. **Порог отсечки** включает карту вероятности превышения, а **Вероятность выше порога** задаёт направление. В дополнительных лежат модель вариограммы баллов, число соседей, радиус поиска и зерно ГСЧ для воспроизводимости.
...
Инструмент **Создать пример для разреза** (группа **Разрез**) сразу готовит данные: шесть стопкой поверхностей с падением и переменной мощностью и линию через площадь. Между поверхностями пять пластов в переслаивании - три вмещающих и два промышленных (2-й и 4-й, тонкие). Кригинг не нужен, поверхности уже растровые. Заодно он выдаёт скважины вдоль линии с полями отметок поверхностей (h1...h6), а также гриды состава промышленных пластов - содержание и минеральный тип с зоной замещения. Запустите его, затем подайте шесть поверхностей сверху вниз (1...6) и линию в **Разрез по линии**, скважины с полями h1...h6 и линию - в **Скважины на разрезе**, а грид состава с кровлей и подошвой промышленного пласта - в **Состав пласта на разрезе**.
...
# Инструмент «Скважины на разрезе»
...
# Инструмент «Состав пласта на разрезе»
...
# Инструмент «Пересечение поверхностей с разрезом»
...
# Инструмент «Пересечение векторов с разрезом»

Если поверхности 3.4 наносятся гридами, то этот инструмент наносит на разрез **векторные** объекты по точному пересечению с линией разреза. Тип результата зависит от объекта.
...
В отличие от **Проекции объектов на разрез** (приблизительной, по коридору) это точное пересечение - метка появляется только там, где геометрия реально режет линию разреза. На вход можно подать **сразу несколько слоёв** (линии и полигоны вперемешку) - все обработаются за один прогон, как список поверхностей в 3.4, а в выходах поле **src** хранит слой-источник каждой метки. Демо-генератор выдаёт разлом, маркер с Z и зону замещения, пересекающие демо-разрез, - на них инструмент пробуется сразу.

# Инструмент «Пересечение TIN с разрезом»
...
# Инструмент «Проекция объектов на разрез»
...
# Инструмент «Спроецировать с разреза»
...
# Инструмент «Развёртка стенки ствола»
