...
=======
![Провайдер Isoliner в панели «Обработка»: четыре группы, тридцать два инструмента.](images/toolbox_tree.png){width=52%}
...
# 1.3 Вариограмма (экспериментальная)
...
# 1.4 Вариограммная карта (анизотропия)
...
# 1.5 Кросс-валидация вариограммы
...
# 1.7 Создать пример скважин (демо)
...
Выход - обычный грид, готовый для **1.2 Изолинии из растра**. В журнал выводятся размер сетки, число узлов-данных, число итераций и итоговая невязка. Если достигнут потолок итераций, а невязка выше порога, инструмент об этом предупреждает.
...
Инструмент отличается от **1.5 Кросс-валидация вариограммы**: та подбирает модель вариограммы для кригинга, а эта сравнивает методы гридирования как таковые и работает в том числе для минимальной кривизны.

Метрики: **ME** (смещение, ближе к 0), **MAE** и **RMSE** (меньше - лучше), **R** (корреляция оценки и факта). Для кригинга дополнительно **MSDR** (ближе к 1, если масштаб стандартной ошибки адекватен).
...
Выход - грид D, который сразу подаётся в **1.2 Изолинии из растра** для изолиний размерности; галка в дополнительных параметрах добавит H вторым каналом. Глобальные D и H по всей поверхности печатаются в журнал.
...
Кровля пласта из кригинга → **2.7** с окном 8 → грид D → **1.2 Изолинии из растра** (канал 1) → изолинии размерности с поясами поверх структурного плана. Глобальное D из журнала - одно число на поверхность для сравнения участков или пластов между собой. Растр должен быть в метрической системе координат; демо-поверхности подходят как есть.
...
На вход - слои **3D-полигонов** (PolygonZ, грани TIN; не треугольники разбиваются веером на треугольники) и при желании **меш-слой**. Линия и vex берутся из определения разреза, высота - с самих граней, поэтому для TIN задавать ничего не нужно. Кроме чертёжной трассы можно получить её и в реальных 3D-координатах.
...
=======
**Скважины (точки)**: выберите слой и отметьте числовые поля отметок, поля вида h1…h6 отмечаются сами. Каждая скважина - вертикальная штанга от минимальной отметки к максимальной с шариком устья на мачте: мачта поднимает устье над кровлей на два процента размаха сцены, скважина видна даже там, где штанга уходит внутрь непрозрачного тела.
>>>>>>> 2dc53d180ea0f6df46ef6849379c188869000699
...
=======
**Плоскость разреза (линия)** принимает любой линейный слой. Удобнее всего подавать **Определение разреза** из инструмента 3.01: лента возьмёт диапазон высот из его полей zmin и zmax. Для произвольной линии лента растягивается на размах сцены с запасом. Ломаные и несколько линий поддерживаются, изломы отрисовываются по вершинам.
...
1. Новый проект QGIS.
2. **1.7 Создать пример скважин (демо)** - появятся демо-скважины с полями отметок и содержания.
