...
![Инструменты модуля в панели «Обработка»: провайдер Isoliner с тремя группами - «1. Грид и изолинии» (1.1-1.7), «2. Дополнительные инструменты анализа» (2.1-2.6) и «3. Разрез» (3.01-3.10) и **«4. Пласт и блочная модель»** (4.01-4.04). Выбор канала растра во всех инструментах - выпадающими списками с именами каналов: у пласта, собранного инструментом 4.01, в списках читаются «кровля», «подошва» и имена слоёв параметров.](images/ui_toolbox.png){width=55%}
...
После установки инструменты появляются в панели «Обработка» (Processing), провайдер «Isoliner», группы «Грид и изолинии», «Дополнительные инструменты анализа» и «Разрез». Требования: QGIS 3.16+. Внешних зависимостей нет - используются NumPy, GDAL и штатные алгоритмы Processing из состава QGIS.
...
- **Окраска** - единый список: Палитра, затем собственные каналы слоя по именам, затем растры проекта. При выборе внешнего растра оживает **Канал атрибута**.
...
<!-- SCREENSHOT: viewer_layers_tab.png | Вкладка «Слои»: фильтр с текстом, кнопки Все/Ничего, список с двумя пластами, ниже панель «Параметры слоя: Пласт 1-й пром.» с раскрытым списком «Окраска» (видны «Палитра», «1 - кровля», «3 - содержание», разделитель, растры проекта) -->
...
**Поле подписи скважин** добавляет текст над мачтами: поля name, well и подобные угадываются автоматически, «(нет)» отключает подписи. Потолок 500 подписей, чтобы крупные фонды не превращали сцену в кашу.

<!-- SCREENSHOT: viewer_well_labels.png | Сцена с телом пласта и скважинами: над мачтами читаются подписи номеров; в кадре левая панель с вкладкой «Векторы» и заполненным «Поле подписи скважин» -->
