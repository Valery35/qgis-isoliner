...
![Инструменты модуля в панели «Обработка»: провайдер Isoliner с тремя группами - «1. Грид и изолинии» (1.1-1.7), «2. Дополнительные инструменты анализа» (2.1-2.6) и «3. Разрез» (3.01-3.10) и **«4. Пласт и блочная модель»** (4.01-4.04).](images/ui_toolbox.png){width=55%}
...
Слева - список растров проекта с галками и настройки, справа - сцена: вращение мышью, зум колесом. Вертикальное преувеличение, разнос по Z и прозрачность применяются мгновенно по кнопке **Обновить сцену**, без пересчёта файлов. Крупные гриды автоматически прореживаются примерно до 60 тысяч узлов.
...
Галка **Тела пластов** включает объёмный режим для многоканальных гридов: канал 1 читается кровлей, канал 2 подошвой, объём замыкается боковой юбкой по границе данных - получается водонепроницаемое тело пласта. Одноканальные растры в том же списке остаются обычными поверхностями, этажерка и тела живут в одной сцене.
...
Тело пласта красится собственным каналом параметра: поле **Канал параметра пласта**, по умолчанию 3 (содержание); значение 4 в демо даёт карту минтипа, 0 возвращает палитру. Одноканальная поверхность параметров не несёт - для неё есть **Окраска поверхностей атрибутом**: внешний растр с выбором канала, классический случай «кровля из кригинга плюс грид содержания из кригинга». Шкала одна на сцену, полоса с диапазоном значений появляется под кнопками; ячейки без данных серые.

## Скважины

Выберите точечный слой и отметьте числовые поля отметок - поля вида h1...h6 отмечаются сами. Каждая скважина - вертикальная штанга от минимальной отметки к максимальной с шариком устья на мачте: мачта поднимает устье над кровлей на два процента размаха сцены, поэтому скважина видна даже тогда, когда штанга целиком уходит внутрь непрозрачного тела. Заглянуть внутрь помогает прозрачность поверхностей.

## Плоскость разреза

Поле **Плоскость разреза (линия)** принимает любой линейный слой. Удобнее всего подавать **Определение разреза** из инструмента 3.01: лента возьмёт диапазон высот из его полей zmin и zmax. Для произвольной линии без этих полей лента растягивается на размах сцены с запасом. Ломаные и несколько линий поддерживаются, изломы отрисовываются по вершинам.
...
Кнопки **Сверху** и **Сбоку** ставят ортогональные виды, **Снимок PNG…** сохраняет кадр сцены в файл - удобно для отчётов и презентаций.

**Клик по поверхности** (без перетаскивания) опрашивает блочную модель: в строке состояния выводятся имя слоя, координаты точки и значения всех каналов, для пласта - ещё и мощность; точка попадания помечается красным шариком. Работает и на телах, и на поверхностях.
