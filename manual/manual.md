...
Смоченный периметр берётся по твёрдым границам, то есть по дну и откосам, без вертикальных плоскостей членения. Методики здесь расходятся, и принято простейшее из соглашений.

## Членение

Задаётся одним из двух способов. Либо створ приходит одной линией, а границы стоят в полях расстояниями по профилю. Либо тремя линиями с полем роли и общим именем - тогда членение живёт в геометрии, как гидрологи и рисуют профили. Второй способ предпочтительнее: расстояния вручную не нужны.
...
Наблюдённые уровни подаются своей таблицей: отметка и подпись. Это замер, а не расчёт, на кривую он не опирается и ложится рядом с расчётными, вида УВ 472.90 X/2021. В слое уровней такие строки помечены kind=obs, расчётные kind=prob, и по этому полю им задаётся разный стиль.
...
Отчёт HTML по каждому створу: профиль с нанесёнными уровнями и границами членения, график зависимости расхода от отметки с линиями обеспеченности, таблица уровней и таблица кривой. Рисунки встроены в саму страницу, поэтому отчёт остаётся одним файлом, который можно переслать.
...
Вместе с профилем переносятся расчётные свойства, если они в таблице есть: границы членения, шероховатости и уклон. Без них вернулась бы геометрия, но не расчёт, и кривая по восстановленным створам разошлась бы с исходной.

# 6.04 Создать пример реки (демо)

Учебная цепочка створов с известным ответом: долина с руслом и двумя поймами, отметки в Z вершин, поля членения, шероховатости, уклона и километража заполнены.
