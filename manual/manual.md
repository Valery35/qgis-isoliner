...
## Цвет полос: палитра Leapfrog

Полосы пластов красятся по имени кровли. Без палитры цвет считается от самого имени и не пляшет между прогонами, но он условный.

Необязательный вход **Палитра кодов Leapfrog (.lfc)** принимает файл легенды с парами код и цвет. Найденные коды красятся её цветами, порядок категорий в легенде идёт по палитре (в ней порядок геологический), а коды вне палитры сохраняют свой прежний цвет. В журнал печатается строка «Цвета полос: из палитры N, своих M» и перечень ненайденных - по ней сразу видно, та ли палитра подана.

Имена слоёв обычно не совпадают с кодами палитры дословно, поэтому поиск идёт лестницей от строгого к терпимому: точный код, затем без учёта регистра и крайних пробелов, затем приведённый вид (снят хвост роли `_top`, `_bottom`, `_кровля`, `_подошва`, латинские двойники букв сложены в кириллицу), и наконец тот же вид без апострофов. Так `KpII_top` находит код пласта `КрII`, а `A'Б_top` находит `АБ`. Строгие шаги идут первыми, поэтому если в палитре есть и `АБ`, и `А'Б` как разные пласты, каждый получит свой цвет.

Тот же файл подаётся в 4.02, и тогда полосы и колонки скважин совпадают по цвету.

Файл палитры читается собственным разбором, без стандартных модулей XML: чужой файл может нести раздутые сущности и внешние ссылки, поэтому объявления DOCTYPE и ссылки на сущности не раскрываются, а у входного файла есть предел размера. Плохой или чужой файл прогон не роняет.
...
| Палитра кодов Leapfrog (.lfc) | Цвета полос из легенды, необязательно. | пусто |
...
## Цвет колонок: палитра Leapfrog

По умолчанию цвет интервала считается от его кода и не меняется между прогонами. Необязательный вход **Палитра кодов Leapfrog (.lfc)** заменяет эти цвета на цвета легенды: найденные коды красятся из палитры, порядок категорий в легенде идёт по ней, коды вне палитры сохраняют прежний цвет. В журнале появляются сводка чтения палитры, строка «Цвета: из палитры N кодов, своих M» и перечень ненайденных кодов.

Поиск цвета терпимый, той же лестницей, что в 4.01 (регистр, хвост роли, латинские двойники букв, апострофы). Плохой или чужой файл прогон не роняет: инструмент пишет предупреждение и красит по-своему.

Палитры бывают разные, и это нормально. Полосы чертежа несут код пласта, а интервалы скважин часто несут породу, поэтому в 4.01 подают стратиграфическую палитру, а в 4.02 - литологическую, и цвета совпадать не должны. Если же в таблице интервалов есть поле с индексом пласта, укажите его как поле кода и подайте ту же палитру, что в 4.01 - тогда колонки сольются с полосами, и на чертеже будет видно только расхождение скважины с построенной поверхностью.
...
| Палитра кодов Leapfrog (.lfc) | Цвета колонок из легенды, необязательно. | пусто |
