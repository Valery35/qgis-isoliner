...
![Инструменты модуля в панели «Обработка»: провайдер Isoliner с тремя группами - «1. Грид и изолинии» (1.1-1.7), «2. Дополнительные инструменты анализа» (2.1-2.6) и «3. Разрез» (3.01-3.10).](images/ui_toolbox.png){width=55%}
...
Готовый учебный набор даёт инструмент **Создать пример для разреза** (3.10): шесть поверхностей сверху вниз, линию, скважины с полями h1...h6 и гриды состава промышленных пластов. Запустите его, затем подайте поверхности и линию сюда, скважины в **Скважины на разрезе**, а грид состава в **Состав пласта на разрезе**. Полный состав набора в разделе 3.10.
...
За один прогон выдаются шесть стопкой поверхностей с падением и переменной мощностью (пять пластов в переслаивании, второй и четвёртый промышленные и тонкие), ломаная линия разреза через площадь, скважины вдоль линии с полями отметок h1...h6 и гриды состава промышленных пластов (содержание и минеральный тип с зоной замещения). Для инструментов пересечения добавлены демонстрационные векторы: разлом без отметки высоты, маркирующий контур с Z, зона замещения и опрокинутая TIN-складка из 3D-граней PolygonZ.
...
| Состав: содержание | Грид содержания промышленных пластов для 3.03. | по запросу |
| Состав: минтип/фации | Грид минтипа (1 сильвинит, 2 замещение) для 3.03. | по запросу |
