...
Isoliner - провайдер инструментов Processing для интерполяции точечных данных и построения изолиний. Ядро кригинга - алгоритм KB2D из GSLIB. Инструменты разнесены по двум группам: «Грид и изолинии» - семь инструментов основного потока обработки, и «Дополнительные инструменты» - пять специализированных расчётов.
...
![Инструменты модуля в панели «Обработка»: провайдер Isoliner с двумя группами - «1. Грид и изолинии» (1.1-1.7) и «2. Дополнительные инструменты» (2.1-2.5).](images/ui_toolbox.png){width=55%}
...
Инструменты собраны в две группы Processing. Группа «Грид и изолинии» это основной поток обработки, от кригинга до изолиний. Группа «Дополнительные инструменты» это специализированные расчёты, категориальный индикаторный кригинг, кригинг с внешним дрейфом, гидравлический градиент с направлением потока, карта вероятности превышения и удельный расход по Дарси.
...
Инструмент стоит в группе **Дополнительные инструменты** и опирается на то же ядро, что и **2D Kriging**. Математика кригинга не меняется. Меняется только то, по чему снимается региональная составляющая.
...
Инструмент стоит в группе **Дополнительные инструменты** и работает как постобработка, подобно гидравлическому градиенту. Своего кригинга он не выполняет и окно **2D Kriging** не трогает, а берёт уже готовые растры. Поэтому он одинаково работает с выходом обычного кригинга и кригинга с внешним дрейфом.
...
Инструмент стоит в группе **Дополнительные инструменты** и работает как постобработка. Свой кригинг не выполняется: растры свойств готовят отдельно кригингом по точкам испытаний.
