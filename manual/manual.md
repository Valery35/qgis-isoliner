...
Выбор канала растра во всех инструментах - выпадающими списками с именами каналов: у пласта, собранного инструментом 5.01, в списках читаются «кровля», «подошва» и имена слоёв параметров.

![Провайдер Isoliner в панели «Обработка».](images/toolbox_tree.png){width=52%}
...
После установки инструменты появляются в панели «Обработка» (Processing), провайдер «Isoliner», группы «Грид и изолинии», «Топография», «Дополнительные инструменты анализа», «Разрез», «Пласт и блочная модель» и «Фрактальный анализ». Требования: QGIS 3.16+. Внешних зависимостей нет - используются NumPy, GDAL и штатные алгоритмы Processing из состава QGIS.
...
- **Озёра** - горизонтальные плоскости. С заполненным полем уровня озеро приколото к нему, без поля уровень берётся автоматически по минимуму прилегающего берега.
...
# 2.04 Заполнение понижений

Заполняет ложные понижения ЦМР методом Планшона-Дарбу, чтобы поток не останавливался в ямах. Понижения в растровых моделях чаще всего артефакты интерполяции и шума, гидрологический анализ без заполнения обрывается в первой же яме.

**Epsilon уклона** управляет режимом. При нулевом значении поднимаются только настоящие ямы ровно до уровня слива, плоские участки остаются плоскими. При положительном значении (по умолчанию 0.001 м) через плоскости дополнительно строится сквозной уклон, и D8 на них становится определён. Для стока и аккумуляции нужен положительный epsilon.

Ячейки на границе грида и рядом с nodata считаются стоками: вода уходит с грида и в вырезы (море). В отчёт печатается число поднятых ячеек и максимальный подъём - удобный индикатор качества исходной ЦМР.
...
![Окно 4.01: линия разреза, поверхности сверху вниз, вертикальный масштаб и выходы чертежа.](images/ui_section_line.png){width=52%}
...
![Окно 4.02: линия, определение разреза, скважины, поля отметок границ и коридор.](images/ui_section_wells.png){width=58%}
...
![Окно 4.03: линия, кровля и подошва пласта, грид состава и режим содержание или класс.](images/ui_section_comp.png){width=55%}
...
![Окно 4.04: определение разреза и список поверхностей-гридов.](images/ui_section_surfaces.png){width=58%}
...
![Окно 4.05: слои для пересечения, чертёж разреза и три выхода (вертикали, точки, полосы зон).](images/ui_section_vectors.png){width=58%}
...
![Окно 4.06: грани TIN из PolygonZ и необязательный меш-слой.](images/ui_section_tin.png){width=58%}
...
![Окно 4.10: экстент, шесть поверхностей с подписями кровля/подошва, линия, скважины и гриды состава.](images/ui_section_demo.png){width=52%}
...
![Конвенция многоканального грида пласта: каналы 1-2 несут геометрию (кровля и подошва), каналы 3 и далее - параметры. Один файл кормит 4.03, 3D-просмотр и 5.04.](images/bed_grid_scheme.png){width=70%}
...
# 5.03 Грид пласта в блочную модель
...
Дальше домен работает как обычный параметр: калькулятор пласта считает по контуру домена, блочная модель фильтруется выражением по коду. Ключевой сценарий - **списание запасов**: посчитайте запасы по контуру до и после погашения и вычтите одно из другого, а разность двух блочных моделей автоматизирует инструмент 5.06.
...
# 5.06 Разность запасов (списание)
...
![Окно 6.04: K, сдвиги сетки и фактор уплотнения в дополнительных параметрах.](images/ui_minkowski.png){width=74%}
...
Цель - убедиться, что связка «домены - блочная модель - разность» считает списание корректно. Проверяются инструменты 5.05, 5.03 и 5.06.

**Шаг A. Домены в канал (5.05).** Грид пласта = «Пласт 1-й пром. (демо)», полигоны = «Зона (демо, полигон)», поле кода пустое. В журнале появится «Ячеек в доменах: N» больше нуля. Откройте результат в 3D-просмотре, окрасьте по каналу **domain** - зона подсветится кодом 1, вне её ноль, граница цвета совпадёт с контуром полигона.

**Шаг D. Сохранение тоннажа при делении (5.03).** Постройте блочную модель из «Пласт 1-й пром. (демо)» с параметром **Слоёв по вертикали = 1**. В таблице атрибутов правой кнопкой по полю **ore_t** откройте статистику и запишите сумму. Постройте ту же модель со **Слоёв = 5** и снова снимите сумму ore_t. Суммы обязаны совпасть до последних знаков, а строк во второй модели станет ровно в пять раз больше. Это ключевая проверка: деление колонки на слои не создаёт и не теряет запас.

**Шаг E. Контрольный ноль (5.06).** Запустите «5.06 Разность запасов», подав одну и ту же модель и как «было», и как «стало», поле ore_t. В журнале должно быть суммарное списание ровно ноль. Так проверяется, что вычитание одинаковых состояний не даёт ложного списания.

**Шаг F. Эмуляция погашения (5.06).** Дублируйте блочную модель (правая кнопка по слою, Дублировать), назовите «стало». В режиме правки обнулите поле ore_t у нескольких центроидов, предварительно записав их суммарный запас как контрольное число. Сохраните правки. Запустите 5.06: «было» - исходная модель, «стало» - изменённая. Суммарное списание в журнале должно равняться обнулённому запасу. У изменённых точек поле **delta** положительное, **before** равно старому значению, **after** равно нулю, у остальных delta равна нулю. Списание внутри произвольного контура получается выделением точек разности полигоном и суммой delta по выделению - это и есть тоннаж, идущий в акт.
...
Открыть журнал можно через **Модули - Isoliner - Журнал** или кнопкой **Журнал** в окне **О плагине**. На панели инструментов **Isoliner** находятся карта плотности, 3D-просмотр поверхностей и окно «О плагине».
