...
![Провайдер Isoliner в панели «Обработка»: пять групп, сорок один инструмент.](images/toolbox_tree.png){width=52%}
...
# 2.07 Плотность по замерам (переменная опора)
...
# 2.08 Создать пример для плотности (демо)

Инструмент создаёт синтетический набор для 2.07 с известной суммарной массой, чтобы проверить инвариант глазами. Десять точек с разными сигмами (от долей ячейки до крупных, масса 500), две линии (масса 200, у одной вырезка интервала from_m/to_m), два полигона (масса 300, один под дазиметрию) и вспомогательный растр. Итого масса 1000.

Прогоните 2.07 на слое точек - интеграл плотности в логе должен дать 500, на линиях 200, на полигонах 300. Поля слоёв: **mass**, **prec**, **from_m**, **to_m**.
...
![Окно 5.04: K, сдвиги сетки и фактор уплотнения в дополнительных параметрах.](images/ui_minkowski.png){width=74%}
...
## Данные для разреза - инструмент 3.10
...
## Плотность (демо) - инструмент 2.08
...
Вспомогательный растр **Вспом. растр (дазиметрия)** - градиент значений для дазиметрического режима 2.07, усл. ед.

## Полиэдр - инструмент 4.07

Полиэдральный слой (3D-грани): поле **bed** (целое) - номер пласта, поле **watertight** (целое) - водонепроницаемость, 1 да или 0 нет.

# Приложение. Плотность переменной опоры: пошагово

Разбор инструментов 2.07 и 2.08 от генерации демо до готового результата, с пояснением каждого параметра. Пример показывает, как проверить инвариант массы глазами.

## Шаг 1. Сгенерировать демо (2.08)

Запустите **2.08 Создать пример для плотности (демо)**.
...
## Шаг 2. Плотность по точкам (2.07)

Запустите **2.07 Плотность по замерам** на слое **Демо-точки**.
...
Запустите 2.07 на слое **Демо-линии**.
...
Запустите 2.07 на слое **Демо-полигоны**.
...
Чтобы собрать точки, линии и полигоны в один растр, прогоните 2.07 трижды подряд.
...
Открыть журнал можно тремя способами: через **Модули - Isoliner - Журнал**, кнопкой **Журнал** на панели инструментов **Isoliner** или кнопкой **Журнал** в окне **О плагине**. На той же панели есть кнопки 3D-просмотра поверхностей и окна «О плагине».
