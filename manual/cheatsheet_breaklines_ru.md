...
# Конвейер: от плотной съёмки до пар бровка-подошва
...
**Шаг 3. Отбор - глазами, а не пересчётом.** Посмотрите в журнал: там процентили перепада по всем кандидатам. Поставьте фильтр слоя `"drop" > 1.0` и двигайте число, глядя на карту. Пересчитывать 2.19 при этом не нужно, а порог, который вы выберете, и есть ваш критерий уступа.

**Шаг 4. Формы - 2.20 Бровки и подошвы в работу.** Вход: кандидаты шага 2 (весь слой, отсечка по перепаду есть и здесь) и та же ЦМР. Предел пути спуска держите близким к заложению откоса. Выход: **Верх** и **Низ** с общим полем **link**, отметки сняты с ЦМР в геометрию Z, плюс слой непарных линий с причиной.
