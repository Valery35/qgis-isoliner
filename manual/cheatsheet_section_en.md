...
{\small QGIS 3.16+ · Isoliner plugin · no GRASS or SAGA}
\end{center}
...
A section in Isoliner is not a picture but a **coordinate system**. The tool **4.01 Cross-section along a line** turns a line on the map into a drawing plane with distance-elevation axes. It produces the drawing of the beds, the surface lines, the axes with a frame, a 3D fence and the **section definition**, a service layer with the `sec_id`, `vex`, `ox` and `oy` fields.

The definition is the key to everything else. All the other tools of the group take it as an input and thanks to it place their objects in the same coordinates as the first drawing. Hence **4.01 first, everything else after**, and the "Section definition" field takes the output of 4.01, not the original line on the map.
...
The most frequent source of confusion. The **Vertical scale** parameter is set in three ways, and they answer different questions.

**Exaggeration factor** - the vertical scale is so many times larger than the horizontal one. Direct and clear, but it requires knowing the length of the section. A factor of 3 on a kilometre-long profile with a fifty-metre range gives a flat ribbon, while on a hundred-metre one it gives an unreadable accordion.

**Scale ratio H:V (1:N)** - the familiar drafting notation, the same number from the other side.

**H:V ratio (drawing width:height)** is the default. The factor is chosen automatically so that the width of the drawing relates to its height as required. This is what is needed in most cases, because a section of any length fits the sheet equally well. A value of 3 or 4 is a reasonable starting point.

**The vertical scale is one for the whole run.** Otherwise the sections would not be comparable with each other.
...
The tool decides what to draw from the geometry of the input. This rule explains why one layer comes out as verticals and another as points.

- **A line without an elevation** (flat in plan: a fault, a boundary, an outline) - a full-height vertical at the crossing station. The place is known, the depth is not.
...
A vertical is correct but poor. The object has an inclination and one wants to see it. When the **dip** and the **dip direction** are given in 4.05, the vertical is replaced by a dip trace and a zone band by a parallelogram.
...
A section across the strike gives the true angle. A section along the strike gives **zero**. The object lies flat, and this is not a failure, since along the strike the bed really is not inclined.
...
**Trace length horizontally** is set in metres (0 = down to the frame). A short pointer trace is usually taken as one and a half to two centimetres on the sheet, at 1:2000 that is 30-40 m.
...
3. **4.01 Cross-section along a line** - the coordinate system of the drawing is built.
4. **4.05 Vector intersection with the section** - the objects land on the section with a real inclination.

Not a single external tool, not a single number taken out of thin air.

# Check it on the demo in five minutes

**4.10 Example for a section (demo)** produces a ready scene. It holds six surfaces, five beds, three section lines, boreholes with intervals, zones, a TIN - and two layers specifically for checking the angles.
...
- **The section came out as a flat ribbon** - the vertical exaggeration was set by a factor without regard to the length. Switch to H:V ratio (drawing width:height).
